...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <form method="POST" action="{{ route('settings.update-pricing') }}" x-data="pricingSettings()">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="markup_hv_persen" x-model.number="markupHv" @input="recalculate()" step="0.1" min="0" max="100"
                           value="{{ $pricing['markup_hv_persen'] }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500">
                ...
                <div>
                    ...
                    <input type="number" name="markup_resep_persen" x-model.number="markupResep" @input="recalculate()" step="0.1" min="0" max="100"
                           value="{{ $pricing['markup_resep_persen'] }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500">
...
function pricingSettings() {
    return {
        ppn: {{ $pricing['ppn_persen'] }},
        markupHv: {{ $pricing['markup_hv_persen'] }},
        markupResep: {{ $pricing['markup_resep_persen'] }},
        hargaJual: 0,
        hargaHV: 0,
        hargaResep: 0,
